...
def gen_tpcds(dir: Path, scale_factor: float, cast_decimal: bool = False):
    ...
    try:
        # 先尝试自动安装
        db.sql("INSTALL tpcds")
        db.sql("LOAD tpcds")
    except Exception as e:
        # 如果自动安装失败，尝试从本地文件加载
        # 你需要先下载tpcds.duckdb_extension文件
        logger.warning("自动安装tpcds扩展失败")
        tpcds_extension_path = "/home/data/tpcds.duckdb_extension/tpcds.duckdb_extension"
        if Path(tpcds_extension_path).exists():
            db.sql(f"LOAD '{tpcds_extension_path}'")
        else:
            # 如果也没有本地文件，尝试使用外部工具
            raise Exception(f"无法加载tpcds扩展。请手动下载tpcds.duckdb_extension：{e}")
