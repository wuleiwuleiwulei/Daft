...
# # ========== 用户配置区域 ==========
# # 设置工作目录 (请根据实际情况修改)
# cd /home/Daft/benchmarking/tpcds
# BENCHMARK_ROOT="/home/Daft/benchmarking/tpcds"

# # 基准测试配置
# export DAFT_RUNNER=native
# PYTHON_BIN=python
# SCRIPT=__main__.py
# TPCDS_GEN_FOLDER=/home/data/tpcds-dbgen
# SCRIPT_PATH="/home/Daft/benchmarking/all_script/test_script"

# ========== 参数检查 ==========
if [ $# -ne 3 ]; then
    echo "用法: $0 <BENCHMARK_ROOT> <TPCDS_GEN_FOLDER> <SCRIPT_PATH>"
    exit 1
fi
...
if [ "$ARCH" = "x86_64" ]; then
    echo "  x86_64架构检测到"
    CPU_BINDING_CONFIGS=(
        # "single_core:numactl --cpunodebind=1 --membind=1 taskset -c 96:--num_cpus 1:CPU96"
...
# 定义要跳过的查询 (已知会失败的)
SKIP_QUERIES=(
    5 10 14 16 23 35 36 39 41 45 47 49
    51 53 54 57 61 63 64 70 72 75 78
    86 87 89 90 94
)

# 基础输出目录
BASE_OUTPUT_DIR="$BENCHMARK_ROOT/tpcds_benchmark_runs_$(date +%Y%m%d_%H%M%S)"
BASE_OUTPUT_DIR_DATA="$BENCHMARK_ROOT/tpcds_benchmark_runs_data_$(date +%Y%m%d_%H%M%S)"
# ================================

echo "开始 TPC-DS 基准测试..."
echo "总计划：运行 $RUN_TIMES 轮，每轮测试 ${#CPU_BINDING_CONFIGS[@]} 种CPU绑定配置。"
echo "将跳过 ${#SKIP_QUERIES[@]} 个已知会失败的查询。"
...
is_skip_query() {
    local q=$1
    for skip in "${SKIP_QUERIES[@]}"; do
        if [[ "$q" -eq "$skip" ]]; then
            return 0  # 返回true，表示跳过
        fi
    done
    return 1  # 返回false，表示不跳过
}
...
for ((run=1; run<=RUN_TIMES; run++)); do
    ...
    for binding_config in "${CPU_BINDING_CONFIGS[@]}"; do
        ...
        for q in $(seq 1 99); do
            # 检查是否跳过此查询
            if is_skip_query "$q"; then
                echo "      跳过查询 Q${q} (已知会失败)"
                continue
            fi
            
            # PERF_DATA_FILE="$CONFIG_DIR/perf_data/perf_q${q}.data"
            PERF_DATA_FILE="$CONFIG_DIR_DATA/perf_data/perf_q${q}.data"
            PERF_REPORT_FILE="$CONFIG_DIR/perf_results/perf_q${q}.txt"
            PERF_REPORT_NOSORT_FILE="$CONFIG_DIR/perf_results/perf_q${q}_nosort.txt"
            PERF_LOG_FILE="$CONFIG_DIR/perf_logs/q${q}_output.log"
            
            echo "      运行查询 Q${q}..."
            
            # 构建基础命令
            BASE_CMD="$PYTHON_BIN -X perf $SCRIPT --tpcds-gen-folder $TPCDS_GEN_FOLDER --cast-decimal --questions $q --scale-factor $SCALE_FACTOR "
            ...
            # 记录开始时间
            echo "=== TPC-DS 查询 Q${q} 开始: $(date) ===" > "$PERF_LOG_FILE"
            ...
            PERF_EXIT_CODE=$?
            echo "==========================================" >> "$PERF_LOG_FILE"
            echo "=== 查询 Q${q} 结束: $(date) ===" >> "$PERF_LOG_FILE"
        ...
        for q in $(seq 1 99); do
            ...
            OUTPUT_JSON_FILE="$CONFIG_DIR/noperf_results/q${q}_benchmark.json"
            OUTPUT_LOG_FILE="$CONFIG_DIR/noperf_logs/q${q}_output.log"
            
            echo "      运行查询 Q${q}..."
            
            # 构建基础命令
            BASE_CMD="$PYTHON_BIN $SCRIPT --tpcds-gen-folder $TPCDS_GEN_FOLDER --cast-decimal --questions $q --scale-factor $SCALE_FACTOR"
            ...
            # 记录开始时间
            echo "=== TPC-DS 查询 Q${q} 开始: $(date) ===" > "$OUTPUT_LOG_FILE"
            ...
            echo "命令: $BASE_CMD" >> "$OUTPUT_LOG_FILE"
            echo "==========================================" >> "$OUTPUT_LOG_FILE"
            
            # 运行无perf的基准测试
            if [ -n "$bind_prefix" ]; then
                # 有绑核命令
                $bind_prefix $BASE_CMD >> "$OUTPUT_LOG_FILE" 2>&1
            else
                # 无绑核命令
                $BASE_CMD >> "$OUTPUT_LOG_FILE" 2>&1
            fi
            
            NOPERF_EXIT_CODE=$?
            echo "==========================================" >> "$OUTPUT_LOG_FILE"
            echo "=== 查询 Q${q} 结束: $(date) ===" >> "$OUTPUT_LOG_FILE"
            echo "退出码: $NOPERF_EXIT_CODE" >> "$OUTPUT_LOG_FILE"
            
            # 尝试生成基准测试JSON结果（如果脚本支持）
            if [ $NOPERF_EXIT_CODE -eq 0 ]; then
                echo "        查询 Q${q} 运行成功"
                
                # 如果有需要，可以添加生成JSON结果的代码
                # 例如：--benchmark-json="$OUTPUT_JSON_FILE"
            else
                echo "        警告: 查询 Q${q} 运行失败，退出码: $NOPERF_EXIT_CODE"
...
echo "有效查询数量: $((99 - ${#SKIP_QUERIES[@]}))"
echo "总测试执行次数: $((RUN_TIMES * ${#CPU_BINDING_CONFIGS[@]} * (99 - ${#SKIP_QUERIES[@]}) * 2))"
echo "========================================="

# 以下开始得出对应的perf xlsx表格和耗时数据表格
python "$SCRIPT_PATH/log_collect.py" "$BASE_OUTPUT_DIR" -o "$BASE_OUTPUT_DIR/tpcds_all_times.xlsx"

python "$SCRIPT_PATH/xlsx_process.py" -i "$BASE_OUTPUT_DIR/tpcds_all_times.xlsx" -o "$BASE_OUTPUT_DIR" -s "multi_core_perf"

python "$SCRIPT_PATH/tpcds_collct.py" "$BASE_OUTPUT_DIR" -o "$BASE_OUTPUT_DIR/tpcds_case_times.xlsx"
...
# 输出txt文件的路径
OUTPUT_PATH="$BASE_OUTPUT_DIR/tpcds_perf"
...
mkdir -p "$BASE_OUTPUT_DIR/tpcds_xlsx" 

# 开始生成perf的xlsx文件
python "$SCRIPT_PATH/perffuncnew_dir_type.py" -i "$OUTPUT_PATH" -o "$BASE_OUTPUT_DIR/tpcds_xlsx"

# 把文件移到同一个位置方便下载
mkdir -p "$SCRIPT_DIR/alltest_result/tpcds"

\cp -f "$BASE_OUTPUT_DIR/tpcds_all_times.xlsx" "$SCRIPT_DIR/alltest_result/tpcds"

\cp -f "$BASE_OUTPUT_DIR/tpcds_case_times_noperf_logs.xlsx" "$SCRIPT_DIR/alltest_result/tpcds"

\cp -f "$BASE_OUTPUT_DIR/tpcds_case_times_perf_logs.xlsx" "$SCRIPT_DIR/alltest_result/tpcds"

\cp -rf "$BASE_OUTPUT_DIR/tpcds_xlsx" "$SCRIPT_DIR/alltest_result/tpcds"

\cp -f "$EXCEL_FILE" "$SCRIPT_DIR/alltest_result/tpcds"