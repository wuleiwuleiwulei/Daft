...
class TpchLogDataCollector:
    ...
    def extract_time_from_log_file(self, log_file: Path) -> Optional[float]:
            ...
            try:
                ...
                # ==== 代码修改开始 ====
                # 1. 首先尝试匹配通用格式："Time taken: 7.747363805770874 seconds" (不含ALL PYTHON前缀)
                # 使用单词边界\b确保前面没有"ALL PYTHON"字样
                # 修改前: pattern = r'Time taken:\s*([\d\.]+)\s*seconds'
                # 修改后: 使用负向先行断言确保前面没有"ALL PYTHON"字样
                general_pattern = r'(?<!ALL PYTHON\s)Time taken:\s*([\d\.]+)\s*seconds'
                ...
                if match:
                    ...
                    # 2. 如果通用格式没找到，尝试备选格式："Execution time: 8.33 seconds."
                    # 修改前: patterns = [r'Execution time:\s*([\d\.]+)\s*seconds']
                    # 修改后: 只保留备选模式
                    backup_pattern = r'Execution time:\s*([\d\.]+)\s*seconds'
                    match = re.search(backup_pattern, content, re.IGNORECASE)
