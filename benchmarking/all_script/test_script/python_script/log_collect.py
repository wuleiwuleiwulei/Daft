...
class TpchLogDataCollector:
    ...
    def extract_query_info_from_filename(self, filename: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        Returns:
            (查询编号, 测试类型)
        """
        # 文件名示例: q17_noperf_output.log 或 q17_perf_output.log
        
        # 提取查询编号
        query_match = re.search(r'q(\d+)', filename, re.IGNORECASE)
        query_num = query_match.group(1) if query_match else filename
                    ...
                    for log_file in log_files:
                        ...
                        if query_num:
                            ...
                            if time_seconds is not None:
                                ...
                                print(f"        提取: Q{query_num} - {time_seconds:.6f} 秒")
                            else:
            ...
            for config in self.configs:
                for test_type in ['noperf', 'perf']:
                    ...
                    if not subset_df.empty:
                        ...
                        # 按Query编号排序
                        pivot_df['Query_Num'] = pivot_df['Query'].str.extract(r'(\d+)').astype(int)
    ...
    def create_query_summary_sheet(self, writer, df):
        ...
        """
        # 找出所有唯一的查询
        queries = sorted(df['Query'].unique(), key=lambda x: int(x[1:]) if x[1:].isdigit() else float('inf'))
    ...
    def print_summary(self):
        ...
        # 按查询统计
        print(f"\n按查询统计 (前10个):")
        for query in sorted(df['Query'].unique(), key=lambda x: int(x[1:]) if x[1:].isdigit() else float('inf'))[:10]:
            query_df = df[df['Query'] == query]
            min_time = query_df['Time (seconds)'].min()
            max_time = query_df['Time (seconds)'].max()
            mean_time = query_df['Time (seconds)'].mean()
