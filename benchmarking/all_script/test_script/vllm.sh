...
# ========== 用户配置区域 ==========
# 设置工作目录 (请根据实际情况修改)
# BENCHMARK_ROOT="/home/Daft/benchmarking/vllm"
# cd /home/Daft/benchmarking/vllm
# ========== 参数检查 ==========
if [ $# -ne 2 ]; then
...
# 基础输出目录
BASE_OUTPUT_DIR="$BENCHMARK_ROOT/vllm_benchmark_runs_$(date +%Y%m%d_%H%M%S)"
BASE_OUTPUT_DIR_DATA="$BENCHMARK_ROOT/vllm_benchmark_runs_data_$(date +%Y%m%d_%H%M%S)"
# ================================

echo "开始 Daft VLLM 基准测试..."
echo "总计划：运行 $RUN_TIMES 轮，每轮测试 ${#CPU_BINDING_CONFIGS[@]} 种CPU绑定配置，每种配置运行 ${#SCRIPTS[@]} 个脚本。"
...
for ((run=1; run<=RUN_TIMES; run++)); do
    ...
    for binding_config in "${CPU_BINDING_CONFIGS[@]}"; do
        ...
        for script_name in "${SCRIPTS[@]}"; do
            # 根据脚本名生成测试名称
            test_name="${script_name%.*}"  # 去掉.py后缀
            if [ -n "$num_cpus_arg" ]; then
                test_name="${test_name}_${config_name}"
            else
                test_name="${test_name}_no_binding"
            fi
            ...
            echo "      测试: $test_name (脚本: $script_name)"
            
            # 构建命令
            BASE_CMD="python -X perf $script_name"
            ...
            echo "测试脚本: $script_name" >> "$PERF_LOG_FILE"
        ...
        for script_name in "${SCRIPTS[@]}"; do
            ...
            echo "        执行命令: $bind_prefix $BASE_CMD"
            ...
            echo "测试脚本: $script_name" >> "$OUTPUT_LOG_FILE"
            ...
            if [ $NOPERF_EXIT_CODE -eq 0 ]; then
                echo "        测试 $test_name 运行成功"
            else
                echo "        警告: 测试 $test_name 运行失败，退出码: $NOPERF_EXIT_CODE"
...
echo "每轮测试脚本数: ${#SCRIPTS[@]}"
echo "总测试执行次数: $((RUN_TIMES * ${#CPU_BINDING_CONFIGS[@]} * ${#SCRIPTS[@]} * 2))"
echo "========================================="

# 以下开始得出对应的perf xlsx表格和耗时数据表格
python "$SCRIPT_PATH/log_collect_name.py" "$BASE_OUTPUT_DIR" -o "$BASE_OUTPUT_DIR/vllm_all_times.xlsx"

python "$SCRIPT_PATH/log_collect_name_case.py" "$BASE_OUTPUT_DIR" -o "$BASE_OUTPUT_DIR/vllm_case_times.xlsx"

python "$SCRIPT_PATH/xlsx_process.py" -i "$BASE_OUTPUT_DIR/vllm_all_times.xlsx" -o "$BASE_OUTPUT_DIR" -s "multi_core_perf"
...
# 输出txt文件的路径
OUTPUT_PATH="$BASE_OUTPUT_DIR/vllm_perf"
...
for perf_file in "${PERF_FILES[@]}"; do
    ...
    # 生成排序版 perf 报告
    # echo "  生成排序版报告..."
    # perf report -i "$perf_file" --stdio -g fractal,0.01 \
    #     --show-total-period --sort=dso > "$SORTED_OUTPUT" 2>&1
    SORTED_EXIT_CODE=0
...
mkdir -p "$BASE_OUTPUT_DIR/vllm_xlsx" 

# 开始生成perf的xlsx文件
python "$SCRIPT_PATH/perffuncnew_dir_type.py" -i "$OUTPUT_PATH" -o "$BASE_OUTPUT_DIR/vllm_xlsx"

# 把文件移到同一个位置方便下载
mkdir -p "$SCRIPT_DIR/alltest_result/vllm"

\cp -f "$BASE_OUTPUT_DIR/vllm_all_times.xlsx" "$SCRIPT_DIR/alltest_result/vllm"

\cp -f "$BASE_OUTPUT_DIR/vllm_case_times.xlsx" "$SCRIPT_DIR/alltest_result/vllm"

\cp -rf "$BASE_OUTPUT_DIR/vllm_xlsx" "$SCRIPT_DIR/alltest_result/vllm"

\cp -f "$EXCEL_FILE" "$SCRIPT_DIR/alltest_result/vllm"